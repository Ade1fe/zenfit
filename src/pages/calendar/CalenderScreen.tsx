...
//       return savedMeals ? JSON.parse(savedMeals) : initialMealEvents
//     }
...
//   // Sync local meal events with parent if provided
//   useEffect(() => {
//     if (initialMealEvents.length > 0 && JSON.stringify(initialMealEvents) !== JSON.stringify(localMealEvents)) {
...
//       onMealEventsChange(localMealEvents)
//     }
//   }, [localMealEvents, onMealEventsChange])
...
//       setCurrentView("meal-planner") // Go back to meal planner after adding
//     }
//   }
...
//       }
//       setLocalMealEvents((prev) => prev.map((meal) => (meal.id === editingMeal.id ? updatedMeal : meal)))
...
//         totalFat: food.fat || 0,
//       }
//       setLocalMealEvents((prev) => [...prev, newMeal])
//     }
...
//                 // If no foods left, remove the meal entirely
//                 return null
//               }
//               return {
//                 ...meal,
...
//     if (isMealEvent) {
//       setLocalMealEvents((prev) => prev.filter((meal) => meal.id !== eventId))
//     } else {
//       setRegularEvents((prev) => prev.filter((event) => event.id !== eventId))
//     }
//   }
...
//     <div className="flex flex-wrap gap-2 mb-6 p-4 bg-white border-b border-gray-100 sticky top-0 z-20">
//       <Button
...
//                           <div
//                             key={event.id}
//                             className={`${eventTypeConfig[event.type].color} text-white rounded-lg p-4`}
//                           >
//                             <div className="flex items-center justify-between">
//                               <div className="flex items-center gap-3">
...
//               </h2>
//               <Button onClick={() => setCurrentView("calendar")} variant="outline" size="sm">
//                 View Full Calendar
//                 <ChevronRight className="w-4 h-4 ml-1" />
//               </Button>
//             </div>
//             <div className="grid grid-cols-7 gap-1 sm:gap-2 overflow-x-auto pb-2 custom-scrollbar">
//               {Array.from({ length: 7 }, (_, i) => {
//                 const date = new Date()
//                 date.setDate(date.getDate() - date.getDay() + i) // Adjust to current week's Sunday
...
//                   <div
//                     key={i}
//                     className={`min-w-[80px] p-3 rounded-lg border-2 text-center ${
//                       isToday
...
//                     </div>
//                     <div className={`text-lg font-bold ${isToday ? "text-emerald-600" : "text-gray-900"} mb-2`}>
//                       {date.getDate()}
//                     </div>
//                     {dayCalories > 0 && (
...
//                   className={`min-h-[100px] p-1 sm:min-h-[120px] sm:p-2 border-r border-b border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors ${
//                     !day.isCurrentMonth ? "bg-gray-50/50 text-gray-400" : ""
//                   } ${day.isToday ? "bg-emerald-50 border-emerald-200" : ""} ${
//                     selectedDate === day.dateString ? "ring-2 ring-emerald-500 ring-inset" : ""
//                   }`}
//                   onClick={() => {
...
//                           <div
//                             key={event.id}
//                             className={`text-xs p-1 rounded ${config.bgColor} ${config.textColor} truncate`}
//                           >
//                             {config.icon} {mealEvent.mealType}
//                           </div>
//                         )
//                       } else {
...
//                           <div
//                             key={event.id}
//                             className={`text-xs p-1 rounded text-white truncate ${regularEvent.color}`}
//                           >
//                             {regularEvent.title}
//                           </div>
//                         )
...
const CalendarApp = ({
  ...
  // Navigation buttons for switching between views
  const NavigationButtons = () => (
    <div className="flex flex-wrap gap-2 mb-6 p-4 bg-white border-b border-gray-100 sticky top-0 z-20 shadow-sm">
      {" "}
      {/* Added shadow-sm */}
      <Button
        onClick={() => setCurrentView("dashboard")}
            ...
            <div className="grid grid-cols-7 gap-1 sm:gap-2 overflow-x-auto pb-2 custom-scrollbar">
              {Array.from({ length: 7 }, (_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`min-w-[80px] p-3 rounded-lg border-2 text-center cursor-pointer hover:shadow-md transition-shadow duration-200
                    ${isToday ? "border-emerald-500 bg-emerald-50 shadow-md" : ""} {/* Added shadow-md for today */}
                    ${!isToday && dayEvents.length > 0 ? "border-gray-300 bg-gray-50" : ""}
                    ${!isToday && dayEvents.length === 0 ? "border-gray-200 bg-white" : ""}
                    `}
                    onClick={() => {
              ...
              {calendarDays.map((day, index) => (
                <div
                  ...
                  className={`
                    ...
                    ${!day.isCurrentMonth ? "bg-gray-50/50 text-gray-400" : ""}
                    ${day.isToday ? "bg-emerald-50 border-emerald-200" : ""}
                    ${selectedDate === day.dateString ? "ring-2 ring-emerald-500 ring-inset shadow-md" : ""} {/* Added shadow-md */}
                    sm:min-h-[120px] sm:p-2
                    ${(index + 1) % 7 !== 0 ? "sm:border-r" : ""} /* Apply right border on sm+ if not the last column */
                  `}
                  onClick={() => {
                    setSelectedDate(day.dateString)
                    setCurrentView("meal-planner") // Navigate to meal planner on day click
        ...
        <main className="p-4 lg:p-6 max-w-7xl mx-auto">
          ...
          <Card className="max-w-xl mx-auto shadow-lg">
            ...
            <div className="space-y-4 p-6">
              ...
              <Button onClick={handleAddOrUpdateRegularEvent} variant="primary" className="w-full">
