...
//     calories: dailyData.reduce((sum, d) => sum + d.calories, 0),
//     protein: dailyData.reduce((sum, d) => sum + d.protein, 0),
//     carbs: dailyData.reduce((sum, d) => sum + d.carbs, 0),
//     fat: dailyData.reduce((sum, d) => sum + d.fat, 0),
//   }
...
//       {
//         label: "Current",
//         data: [last7DaysTotals.protein, last7DaysTotals.carbs, last7DaysTotals.fat],
//         backgroundColor: ["rgba(34, 197, 94, 0.8)", "rgba(251, 191, 36, 0.8)", "rgba(168, 85, 247, 0.8)"],
//         borderColor: ["rgb(34, 197, 94)", "rgb(251, 191, 36)", "rgb(168, 85, 247)"],
//         borderWidth: 2,
//         borderRadius: 6,
...
//       {
//         label: "Goal",
//         data: [goals.protein * 7, goals.carbs * 7, goals.fat * 7],
//         backgroundColor: ["rgba(34, 197, 94, 0.2)", "rgba(251, 191, 36, 0.2)", "rgba(168, 85, 247, 0.2)"],
//         borderColor: ["rgb(34, 197, 94)", "rgb(251, 191, 36)", "rgb(168, 85, 247)"],
//         borderWidth: 1,
//         borderRadius: 6,
...
//             <div className="lg:col-span-2">
//               <h3 className="text-lg font-semibold text-gray-800 mb-1">7-Day Calorie Trend</h3>
//               <p className="text-sm text-gray-600 mb-4">Track your daily calorie intake</p>
//               <div className="h-64">
//                 <Line data={lineChartData} options={lineChartOptions} />
//               </div>
//             </div>
...
//             <div className="flex flex-col">
//               <h3 className="text-lg font-semibold text-gray-800 mb-1">Today's Progress</h3>
//               <p className="text-sm text-gray-600 mb-4">Daily calorie goal tracking</p>
//               <div className="flex-1 flex flex-col items-center justify-center">
//                 <div className="relative h-48 w-48">
//                   <Doughnut data={doughnutData} options={doughnutOptions} />
//                   <div className="absolute inset-0 flex flex-col items-center justify-center">
...
//             <div className="flex items-center space-x-2">
//               <Target className="h-5 w-5" />
//               <CardTitle className="text-lg sm:text-xl">Macronutrients (7 Days)</CardTitle>
//             </div>
//           </div>
//         </CardHeader>
//         <CardContent className="p-6">
//           <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
//             <div>
//               <h3 className="text-lg font-semibold text-gray-800 mb-1">Weekly Macros</h3>
//               <p className="text-sm text-gray-600 mb-4">Current vs goal breakdown</p>
//               <div className="h-64">
//                 <Bar data={macrosBarData} options={barChartOptions} />
//               </div>
//             </div>
...
//                 const label = macro[0].toUpperCase() + macro.slice(1)
//                 const current = last7DaysTotals[macro as keyof typeof last7DaysTotals]
//                 const target = goals[macro as keyof typeof goals] * 7
//                 const percent = Math.min((current / target) * 100, 100)
//                 const barColor = {
//                   protein: "green",
//                   carbs: "yellow",
//                   fat: "purple",
//                 }[macro as "protein" | "carbs" | "fat"]

//                 return (
//                   <div key={macro} className={`p-4 bg-${barColor}-50 rounded-lg border border-${barColor}-100`}>
//                     <div className="flex justify-between items-center mb-2">
//                       <span className={`font-medium text-${barColor}-800`}>{label}</span>
//                       <span className={`text-sm text-${barColor}-600`}>
//                         {current}g / {target}g
//                       </span>
//                     </div>
//                     <div className={`w-full bg-${barColor}-200 rounded-full h-2`}>
//                       <div
//                         className={`bg-${barColor}-500 h-2 rounded-full transition-all duration-500`}
//                         style={{ width: `${percent}%` }}
//                       />
//                     </div>
//                     <div className={`text-xs text-${barColor}-600 mt-1`}>{Math.round(percent)}% complete</div>
//                   </div>
//                 )
//               })}
//             </div>
...
export default function CaloriesGraph({ mealEvents = [] }: CaloriesGraphProps) {
    ...
    datasets: [
      {
        ...
        data: dailyData.map((d) => d.calories),
        borderColor: "000",
        backgroundColor: "rgba(29, 25, 29, 0.1)",
        fill: true,
        tension: 0.4,
  ...
  const macrosBarData = {
    ...
    datasets: [
      ...
      },
      {
        label: "Goal",
        data: [goals.protein * 7, goals.carbs * 7, goals.fat * 7],
        backgroundColor: ["#221e1ef1", "#221e1ef1", "#221e1ef1"],
        borderColor: ["000", "000", "000"],
        borderWidth: 1,
        borderRadius: 6,
        borderSkipped: false,
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        data: [todayTotals.calories, Math.max(0, goals.calories - todayTotals.calories)],
        backgroundColor: ["000", "rgba(229, 231, 235, 0.3)"],
        borderColor: ["000", "rgba(229, 231, 235, 0.5)"],
        borderWidth: 2,
        cutout: "70%",
      },
    ],
            ...
            <div className="flex flex-col">
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                <div className="relative h-48 w-48">
                  <Doughnut data={doughnutData} options={doughnutOptions} />
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-bold text-black">{todayTotals.calories}</span>
                    <span className="text-sm text-gray-500">/ {goals.calories}</span>
                    <span className="text-xs text-gray-400 mt-1">{Math.round(calorieProgress)}% complete</span>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              {["protein", "carbs", "fat"].map((macro) => {
                ...
                const percent = Math.min((current / target) * 100, 100)
                return (
                  <div key={macro} className="p-4 bg-gray-50 rounded-lg border border-grey-300">
                    <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="text-xs text-black mt-1">{Math.round(percent)}% complete</div>
                  </div>
                )
              })}
            </div>
